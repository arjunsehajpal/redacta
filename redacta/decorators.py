...
def pii_protect_anthropic_messages(
    pipeline: Optional[Pipeline] = None,
    *,
    verbose: Optional[bool] = None,
) -> Callable[[Callable], Callable]:
    """Decorator to add PII protection to Anthropic messages API calls."""
    default_verbose = True if verbose is None else verbose
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            messages = extract_anthropic_messages_from_kwargs(kwargs)

            if messages is None:
                return func(*args, **kwargs)

            sanitized_messages, chat_result = sanitize_anthropic_messages(messages, pipeline)

            set_anthropic_messages_in_kwargs(kwargs, sanitized_messages)
            ...
            if is_streaming and response is not None:
                ...
                return restore_anthropic_streaming_response(response, pipeline, chat_result)

            output_text = get_anthropic_output_text(response)
            ...
            if output_text is not None:
                restored_text = pipeline.restore_response(output_text, sanitized_result_for_restore)
                set_anthropic_output_text(response, restored_text)
